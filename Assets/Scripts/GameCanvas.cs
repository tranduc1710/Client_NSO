...
public class GameCanvas : IActionListener
{
	...
	{
		return mSystem.getCurrentTimeMillis() - lastTimePress >= 800;
	}
	...
	public static bool isPointerInRoll(int x, int y, int w, int h, Scroll scroll)
	{
		...
		if ((isPointerDown || isPointerJustRelease) && num >= x && num <= x + w && num2 >= y)
		{
			return num2 <= y + h;
		}
		return false;
	...
	public static void paintBGGameScr(mGraphics g)
	{
		if (GameScr.CheDoDem)
		{
			//g.drawImage(GameScr.night, w / 2, h / 2, 3);
		}
		else if (isBallEffect)
		{
			if (gameTick % 10 > 7)
			{
				g.setColor(16777215);
	...
	{
		int y2 = ((!noRotate) ? (gameTick % 3) : 0) * 16;
		g.drawRegion(imgShuriken, 0, y2, 16, 16, 0, x, y, mGraphics.HCENTER | mGraphics.VCENTER);
	}
	...
	public void perform(int idAction, object p)
	{
		...
		case 8880:
			...
			break;
		case 8881:
			NinjaUtil.downloadGame((string)p);
			currentDialog = null;
			break;
		case 8882:
			currentDialog = null;
		...
		case 8883:
		{
			...
			NinjaUtil.sendMsDK((string)((MyVector)p).elementAt(0), port);
			currentDialog = null;
			break;
		}
		case 8884:
		...
		case 88812:
		{
			Char char2 = (Char)p;
			endDlg();
			Service.gI().acceptInviteTest(char2.charID);
			break;
		}
		case 88813:
		{
			endDlg();
			Item[] items = (Item[])p;
			Service.gI().crystalCollect(items);
			break;
		}
		case 88814:
		{
			Item[] items2 = (Item[])p;
			endDlg();
			Service.gI().crystalCollectLock(items2);
			break;
		}
		case 88815:
		...
		case 88835:
		{
			int num4 = int.Parse((string)p);
			int num5 = int.Parse(inputDlg.tfInput.getText());
			currentDialog = null;
			if (num5 < 1 || num5 >= Char.getMyChar().arrItemBag[num4].quantity)
			{
				startOKDlg(mResources.INVALID_NUMSPLIT);
			}
			else
			{
				Service.gI().inputNumSplit(num4, num5);
			}
			break;
		}
		case 88836:
		...
		case 88841:
		{
			Char char3 = (Char)p;
			endDlg();
			Service.gI().acceptInviteTestGT(char3.charID);
			break;
		}
		case 88843:
		{
		...
		case 88834:
		{
			...
			try
			{
				...
				if (num3 > 0)
				{
					Service.gI().outputCoinClan(num3);
				}
				break;
			}
			catch (Exception)
		...
		case 88838:
		{
			string text2 = input2Dlg.tfInput.getText().Trim();
			string text3 = input2Dlg.tfInput2.getText().Trim();
			endDlg();
