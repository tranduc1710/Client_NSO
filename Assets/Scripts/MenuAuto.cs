	...
	public void perform(int idAction, object p)
	{
		...
		switch (idAction)
		{
			...
			case 53:
				try
				{
					string path = "Assets/Resources/HuongDan.txt";
					string helpText = File.ReadAllText(path);
					GameScr.gI().showAlert("Lệnh Chat", helpText, withMenuShow: true);
				}
				catch (FileNotFoundException)
				{
					GameScr.gI().showAlert("Lỗi", "Không tìm thấy file Data/HuongDan.txt!", withMenuShow: true);
				}
				catch (IOException ex)
				{
					GameScr.gI().showAlert("Lỗi", "Không thể đọc file hướng dẫn:\n" + ex.Message, withMenuShow: true);
				}
				break;
		}
	}
